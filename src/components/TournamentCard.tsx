        ...
        <div className="card-modern group relative flex flex-col h-full bg-slate-900/30 backdrop-blur-md border border-white/5 rounded-[2rem] overflow-hidden">
            ...
            {showActions && (
                <div className="absolute top-4 left-4 z-40 flex flex-col gap-2">
                    <button onClick={onEdit} className="p-2.5 bg-indigo-500/90 hover:bg-indigo-600 text-white rounded-xl backdrop-blur-md shadow-lg transition-all active:scale-95 group/edit">
                    ...
                    <button onClick={onDelete} className="p-2.5 bg-red-500/90 hover:bg-red-600 text-white rounded-xl backdrop-blur-md shadow-lg transition-all active:scale-95">
            ...
            <div className="absolute top-6 right-6 z-30">
                <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.15em] shadow-xl border ${statusColor} backdrop-blur-xl ring-4 ring-black/10`}>
                    {statusText}
                </span>
            </div>

            {/* Dynamic Visual Header */}
            <div className={`relative h-60 w-full bg-gradient-to-br ${getSportGradient(tournament.sport)} transition-transform duration-700 group-hover:scale-105`}>
                <div className="absolute inset-0 bg-black/20" />
                <div className="absolute bottom-0 left-0 w-full h-32 bg-gradient-to-t from-slate-950 via-slate-950/40 to-transparent" />

                {/* Floating Sport Icon */}
                <div className="absolute inset-0 flex items-center justify-center opacity-20 pointer-events-none">
                    <SportIcon sport={tournament.sport} className="w-48 h-48 text-white blur-[1px]" />
                </div>

                {/* Sport Tag */}
                <div className="absolute bottom-6 left-6 flex items-center gap-2 px-3 py-1.5 bg-white/10 backdrop-blur-md border border-white/20 rounded-xl">
                    <SportIcon sport={tournament.sport} className="w-4 h-4 text-white" />
                    <span className="text-[10px] font-bold text-white uppercase tracking-[0.2em]">{tournament.sport}</span>
                </div>
            </div>
            ...
            <div className="p-8 flex flex-col flex-grow bg-slate-950/40">
                <div className="mb-6">
                    <h3 className="text-3xl font-black text-white line-clamp-2 leading-none tracking-tighter mb-2">
                        {tournament.title}
                    </h3>
                    <div className="w-12 h-1 bg-indigo-500 rounded-full" />
                </div>

                <div className="grid grid-cols-1 gap-5 text-sm font-medium text-slate-400 mb-8">
                    <div className="flex items-center gap-4">
                        ...
                        <span className="text-slate-200">{format(new Date(tournament.date), 'MMMM dd • h:mm a')}</span>
                    </div>

                    <div className="flex items-center gap-4">
                        <div className="w-10 h-10 flex items-center justify-center bg-pink-500/10 rounded-xl border border-pink-500/20">
                            <svg className="w-5 h-5 text-pink-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /></svg>
                        </div>
                        <span className="text-slate-200 truncate">{tournament.venue}</span>
                    </div>
                </div>
                ...
                <div className="mt-auto pt-6 border-t border-white/5">
                    {showActions ? (
                        <div className="flex flex-col gap-3">
                            <Link to={`/admin/registrations/${tournament.id}`} className="w-full py-4 px-6 bg-slate-800 hover:bg-slate-700 text-white text-center text-xs font-black uppercase tracking-widest rounded-2xl transition-all shadow-lg active:scale-95">
                                View Registrations
                            </Link>
                            <Link to={`/admin/leaderboard/${tournament.id}`} className="w-full py-4 px-6 bg-amber-500/10 border border-amber-500/20 hover:bg-amber-500/20 text-amber-500 text-center text-xs font-black uppercase tracking-widest rounded-2xl transition-all shadow-lg active:scale-95">
                                Result Dashboard
                            </Link>
                        </div>
                    ) : (
                        <div className="flex gap-3">
                            <Link
                                ...
                                className="flex-1 py-4 px-6 bg-white/5 hover:bg-white/10 border border-white/5 text-white text-center text-sm font-bold rounded-2xl transition-all"
                            >
                                Details
                            </Link>
                            {!isClosed && !isExpired && (
                                <Link
                                    ...
                                    className="flex-[2] py-4 px-6 bg-gradient-to-r from-indigo-500 to-purple-500 hover:shadow-[0_0_20px_-5px_rgba(99,102,241,0.5)] text-white text-center text-sm font-black uppercase tracking-wider rounded-2xl transition-all active:scale-95"
                                >
                                    Register Now
                                </Link>
                            )}
